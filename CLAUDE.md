...
- [backend/lambda/rag_pipeline/lambda_function.py](backend/lambda/rag_pipeline/lambda_function.py) — dispatcher in `lambda_handler` routes by `action`:
  ...
  - `propose`: identify high-probability research directions from the ingested corpus. Retrieves with optional `topic` focus, asks one LLM call to generate candidates with strict grounding rules (≥2 citations per path, must fit one of 5 high-value categories: contradiction / extension / mechanism / combination / gap), drops candidates that don't meet the bar, then layers quantitative scoring on top: **novelty** (1 − cosine similarity of claim embedding to corpus centroid) and **convergence** (mean cosine similarity to retrieved chunks). Final ranking blends evidence strength, impact, convergence, and novelty. Returns one structured `ResearchPath` per direction with rationale, builds-on, open question, suggested approach, why-now, risks, and scores. UI renders each as a card with a "Copy" button producing a markdown block the user can paste into any downstream workflow.
